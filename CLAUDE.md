...
CI (`.github/workflows/ci.yml`) has five jobs: lint (fmt + clippy, ubuntu only), test matrix (Linux + macOS, plus a release-profile build on ubuntu to catch LTO breakage), MSRV (`rust-version` in Cargo.toml, currently 1.88 — floor set by let-chains in our code and comfy-table), `audit` (`cargo audit` against the RUSTSEC advisory db), and `deny` (`cargo deny check` — advisories, licenses, bans, sources; config in `deny.toml`). Everything runs `--locked` — keep `Cargo.lock` committed. The release workflow builds 6 targets, tests each, then a single `publish` job uploads all artifacts plus a combined `SHA256SUMS` atomically (never let per-matrix-job release uploads back in — partial releases). `ci.yml` uses floating major-version action tags (`@v4`, `@stable`, `@v2`); the release workflows (`release.yml`, `release-plz.yml`) likewise use floating tags (`@v4`, `@stable`, `@v0.5`) to stay aligned with the env-shield publishing setup they were ported from.
...
- Ownership has two modes: full-history shortlog semantics by default, but when the user narrows the path set (`--path`, auto-scope, or `--exclude-path`) it switches to scoped mode (`Report::ownership_scoped`) — only authors whose window commits touched included paths, computed in `metrics::scoped_author_stats`. Default noise filters alone must NOT trigger scoped mode (`PathFilter::is_restrictive`).
